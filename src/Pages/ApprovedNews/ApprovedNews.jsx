...
const ApprovedNews = () => {
  ...
  const handleTopAction = (id, item) => {
    Swal.fire({
      title: 'Select Top News Type',
      input: 'select',
      inputOptions: {
        topHead: 'Set Top Head News',
        topNews: 'Set Top News',
        none: 'None'
      },
      ...
      if (result.isConfirmed) {
        let payload = { isTopHead: false, isTopNews: false };

        if (result.value === 'topHead') {
          payload.isTopHead = true;
        } else if (result.value === 'topNews') {
          payload.isTopNews = true;
        } else if (result.value === 'none') {
          payload.isTopHead = false;
          payload.isTopNews = false;
        } else {
          payload.isTopHead = item.isTopHead;
          payload.isTopNews = item.isTopNews;
        }

        try {
          await axios.patch(`https://nishibarta-server.vercel.app/news/top/${id}`, payload);
          // Update local state
          setNews((prevNews) =>
          ...
          Swal.fire('Success!', 'News status updated successfully.', 'success');
        } catch (err) {
          Swal.fire('Failed', 'Something went wrong.', 'error');
        }
      }
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Approved News</h2>
      ...
      <div className="overflow-x-auto hidden md:block">
        <table className="w-full border text-center">
          ...
          <tbody>
              ...
              <tr key={item._id}>
                ...
                <td className="border p-2">
                  <button
                    ...
                    className="bg-green-400 text-white px-3 py-1 rounded font-semibold"
                  >
                    Approved
                  </button>
                </td>
                <td className="border p-2">
                  <button
                    onClick={() => handleTopAction(item._id, item)}
                    className={`text-white px-3 py-1 rounded font-semibold ${item.isTopHead
                      ? 'bg-green-700'
                      : item.isTopNews
                        ? 'bg-green-400'
                        : 'bg-gray-400'
            ...
            {news.length === 0 && (
              <tr><td colSpan="7" className="text-center py-4">No news found</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="space-y-4 md:hidden">
        {news.map(item => (
          <div key={item._id} className="border rounded shadow p-3">
            ...
            <button
              onClick={() => handleTopAction(item._id, item)}
              className={`flex-1 text-white py-1 rounded w-full mt-3 ${item.isTopHead
                ? 'bg-green-700'
                : item.isTopNews
                  ? 'bg-green-400'
                  : 'bg-gray-400'
            ...
            <div className="flex gap-2 mt-3">
              <button
                onClick={() => handleStatusAction(item._id)}
                className="flex-1 bg-green-400 text-white py-1 rounded"
              >
                Approved
              </button>
              <button
        ...
        {news.length === 0 && (
          <p className="text-center py-4">No news found</p>
        )}
      </div>
